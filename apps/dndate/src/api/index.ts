...
API.post("poll/create-user", async (ctx) => {
    ...
    if ((reqData.pass ?? "") != "") {
        pass = Bun.SHA512.hash(Bun.env.APP_ID + reqData.pass, "hex");
    }  
    ...
    if ((reqData.pass ?? "") == "") {
        token = Bun.SHA256.hash(Bun.env.APP_ID + moment().toISOString() + "-" + newUser[0]?.id, "hex");
        tempToken[reqData.token + token] = moment().add(1, "h").valueOf();
    }
