...
import { faCalendarDays, faCircleInfo, faDiceD20, faLeaf, faPenToSquare, faPersonChalkboard, faShare, faTrash, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
...
export const ScheduleTable: FC<Props> = memo(({ 
    ...
    const TimeslotCell = (date: string, timeslotIdx: number, user: UserData) => {
        ...
        if (timeslotShift > 0) {
            if ((date == dates[dates.length - 1]) && (timeslotIdx >= timeslotShift)) {
                attType += " disabled"
            } else if ((date == dates[0]) && (timeslotIdx < timeslotShift)) {
                attType += " disabled"
            }
        } else if (timeslotShift < 0) {
            if ((date == dates[0]) && (timeslotIdx < 48 + timeslotShift)) {
                attType += " disabled"
            } else if ((date == dates[dates.length - 1]) && (timeslotIdx >= 48 + timeslotShift)) {
                attType += " disabled"
            }
        } 
        
        if (!attType.includes("disabled")) {
        ...
        if (!attType.includes("cursor-not-allowed")) {
            attType += " cursor-pointer";
        }
            ...
            >
                {
                    (attType.includes("host-unavail") && (!user.host)) ? 
                        <div className='flex flex-row opacity-75'>
                            <FontAwesomeIcon icon={faTriangleExclamation} className='mx-auto' />
                        </div>                    
                    :
                    null
                }                
                            ...
                            <tr>
                                { dates.map((date, dateIdx) => {
                                    return Array.from(Array(24*2).keys()).map((timeslotIdx) => (
                                        ...
                                        >
                                            <div className="absolute ps-2 top-0" style={{ zIndex: 4 }}>{(timeslotIdx % 2 == 0) ? timeslotIdx/2 : ""}</div>                                                        
                                        </th>
                                    ))
                                })}
                            </tr>
                                ...
                                <Fragment key={'v-' + date}>
                                    ...
                                    <tr>
                                        ...
                                        { Array.from(Array(24*2).keys()).map((timeslotIdx) => (
                                            ...
                                            >
                                                <div className="absolute ps-2 pb-1 bottom-0" style={{ zIndex: 4 }}>{(timeslotIdx % 2 == 0) ? timeslotIdx/2 : ""}</div>                                                        
                                            </th>
                                        ))}
                                    </tr>
                                    {userData.map((user, idx) => (
